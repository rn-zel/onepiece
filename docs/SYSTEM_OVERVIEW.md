...
├── package.json
├── tsconfig.json
├── backend/                   # Laravel backend project (Herd linked)
├── index.html
│
├── docs/
│   ├── ARCHITECTURE.md        # Main architecture reference
│   ├── LARAVEL_INTEGRATION.md # Laravel setup and integration guide
│   └── README.md              # Doc index
│
└── src/
...
- **With Laravel Herd:** Link the `backend/` folder to Herd. Set `API_BASE_URL` in Config to `http://backend.test/api`. Use Laravel Controllers and Services for session and RNG logic.
