...
export class SpinOrchestrator {
  ...
  animateReels(
    ...
    this.spinAuraSprite = this.particleEmitter.emitAura(
      this.uiManager.spinButton,
    );
    ...
    const btnScale = isPortrait
      ? CONFIG.SPIN_BTN_PORTRAIT_SCALE
      : CONFIG.SPIN_BTN_LANDSCAPE_SCALE;

    gsap.to(this.uiManager.spinButton.scale, {
      x: btnScale,
      y: btnScale,
      duration: 0.4,
      ease: "back.out(2)",
    ...
    this.reels.forEach((r, i) => {
      ...
      gsap.to(r, {
        ...
        onUpdate: () => {
          ...
          r.blur.strengthY =
            remaining <= CONFIG.REEL_BLUR_FADE_DIST
              ? Math.max(
                  0,
                  (remaining / CONFIG.REEL_BLUR_FADE_DIST) *
        ...
        onComplete: () => {
          ...
          gsap.fromTo(
            ...
            {
              y: originalY,
              duration: CONFIG.REEL_BOUNCE_SPEED,
              ease: "back.out(1.5)",
            },
          );
            ...
            if (this.spinAuraSprite) {
              gsap.to(this.spinAuraSprite, {
                ...
                onComplete: () => {
                  if (this.spinAuraSprite) {
                    gsap.killTweensOf(this.spinAuraSprite);
                    this.spinAuraSprite.parent?.removeChild(
                      this.spinAuraSprite,
                    );
                    this.spinAuraSprite.destroy();
  ...
  getVisibleGrid(): number[][] {
    return this.reels.map((reel) =>
      [0, 1, 2].map((row) =>
        this.slotTextures.indexOf(reel.getSymbolAtRow(row).texture),
      ),
    );
  }
