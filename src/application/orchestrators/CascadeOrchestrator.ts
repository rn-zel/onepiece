...
import type { CascadePlayStep, GridPosition, SymbolSprite } from "../../domain/models/GameTypes";
...
export class CascadeOrchestrator {
    ...
    async play(
        ...
        for (const step of steps) {
            ...
            for (const p of winningPositions) {
                ...
                this.symbolAnimator.play(symbolIndex, sprite, reel, this.activeAnimations, false);
                ...
                if (this.activeAnimations.length > previousCount) {
                    const addedAnim = this.activeAnimations[this.activeAnimations.length - 1] as any;
            ...
            for (const p of winningPositions) {
                ...
                if (!reel) continue;
                const sprite = reel.getSymbolAtRow(p.row);
                const baseScale = (sprite as unknown as SymbolSprite).baseScale || sprite.scale.x || 1;
                ...
                const animIndex = this.activeAnimations.findIndex((a: any) => a._cascadeReel === p.reel && a._cascadeRow === p.row);
                if (animIndex !== -1) {
                    const anim = this.activeAnimations[animIndex];
                    this.activeAnimations.splice(animIndex, 1); // remove from array
                    breakTweens.push(gsap.to(anim, { 
                        alpha: 0, 
                        duration: 0.25, 
                        ease: "power2.out",
                        onComplete: () => {
     ...
     */
    private _buildSteps(cascaded: BackendCascadeStep[], initialWin: number): CascadePlayStep[] {
    ...
    private _getVisibleGrid(): number[][] {
        return this.reels.map((reel) =>
            [0, 1, 2].map((row) => reel.slotTextures.indexOf(reel.getSymbolAtRow(row).texture))
        );
    }
    ...
    private async _animateDrop(
        ...
        for (const [reelIndex, rows] of removedByReel) {
            removedByReel.set(reelIndex, Array.from(new Set(rows)).sort((a, b) => a - b));
        }
        ...
        for (let reelIndex = 0; reelIndex < this.reels.length; reelIndex++) {
            ...
            const keptOldRows = ([0, 1, 2] as const).filter((r) => !removedRows.includes(r));
    ...
    private _spawnWinChip(
        ...
        gsap.to(winContainer, {
            alpha: 0, duration: 0.5, delay: CONFIG.CASCADE_WIN_DELAY,
        ...
        if (multiplier > 1) {
            ...
            gsap.to(multContainer.scale, {
                x: 1, y: 1, duration: CONFIG.WIN_TEXT_POPUP_SPEED,
                delay: CONFIG.CASCADE_MULT_SPAWN_DELAY, ease: "back.out(2)",
            });
