...
/** Format bet for display (e.g. "300.00"). */
export function formatBetAmount(amount: number): string {
    return amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

/** Total bet values derived from bet sizes (e.g. 10 × 30 = 300). */
...
const SHARED = {
    ...
    /** Background glow color for each tier (hex). Big=blue, Mega=gold, Max=red. */
    TIER_BG_GLOW: {
        big: 0x29b6f6,
        mega: 0xffc107,
        max: 0xe91e63,
    } as const,
    /** Text fill color for each tier (hex). */
    TIER_TEXT_FILL: {
        big: 0x29b6f6,
        mega: 0xfbff00,
        max: 0xe91e63,
    ...
    BET_PRESETS: [BET_VALUES_FROM_RULES[0] ?? 300, BET_VALUES_FROM_RULES[4] ?? 3000, BET_VALUES_FROM_RULES[6] ?? 6000, BET_VALUES_FROM_RULES[10] ?? 30000],
    PARTICLE_ORIGIN_X: 0,
...
function insertLandscapeKey(shortKey: string): string {
    ...
    return shortKey.slice(0, lastUnderscore) + "_LANDSCAPE_" + shortKey.slice(lastUnderscore + 1);
}
function insertPortraitKey(shortKey: string): string {
    const lastUnderscore = shortKey.lastIndexOf("_");
    if (lastUnderscore <= 0) return shortKey + "_PORTRAIT";
    return shortKey.slice(0, lastUnderscore) + "_PORTRAIT_" + shortKey.slice(lastUnderscore + 1);
}
...
function applyHudScaleAliases(flat: Record<string, unknown>): void {
    (flat as any).HUD_BAL_LANDSCAPE_BG_SCALE = (LANDSCAPE as any).HUD_BAL_BG_SCALE;
    (flat as any).HUD_BAL_PORTRAIT_BG_SCALE = (PORTRAIT as any).HUD_BAL_BG_SCALE;
    (flat as any).HUD_BET_LANDSCAPE_BG_SCALE = (LANDSCAPE as any).HUD_BET_BG_SCALE;
...
export const CONFIG = buildFlatConfig() as typeof SHARED & { LANDSCAPE: typeof LANDSCAPE; PORTRAIT: typeof PORTRAIT } & { [k: string]: any };
