...
export class HelpModal {
  ...
  private renderPayoutsAndWays() {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (sym === "wild") {
        const txt = new Text({
          text: "Substitutes for all except Scatter.",
          style: new TextStyle({
            fill: 0xffffff,
            fontSize: 14,
            wordWrap: true,
        ...
        const txt = new Text({
          text: "3/4/5 Scatters = Free Spins. 3x=20, 4x=50, 5x=200",
          style: new TextStyle({
            fill: 0xffffff,
            fontSize: 14,
            wordWrap: true,
        ...
        if (payouts) {
          ...
          const t4 = new Text({
            text: `4x: ${payouts[4]}`,
            style: new TextStyle({ fill: 0xdddddd, fontSize: 14 }),
          });
          const t3 = new Text({
            text: `3x: ${payouts[3]}`,
            style: new TextStyle({ fill: 0xbbbbbb, fontSize: 14 }),
          });
    ...
    const subtitle = new Text({
      text: "Left to Right Winning Sequence",
      style: new TextStyle({
        fill: 0xffffff,
        fontSize: 18,
        fontWeight: "bold",
  ...
  public handleResize(width: number, height: number) {
    if (this.modalContainer) {
      ...
      const frameWidth = PANEL_WIDTH;
      const frameHeight = PANEL_HEIGHT;

      const scaleX = (width * 0.9) / frameWidth;
      const scaleY = (height * 0.9) / frameHeight;
      const fitScale = Math.min(scaleX, scaleY);

      this.modalContainer.scale.set(fitScale);
    }
  }
}
